...
export const useGradebookTableData = () => {
  ...
  const mapHeaders = (heading) => {
    ...
    } else if (heading === Headings.lastUpdated) {
      label = <LabelReplacements.MastersOnlyLabelReplacement {...messages.lastUpdatedHeading} />;
    } else {
      label = heading;
    }
  ...
  const mapRows = entry => ({
    ...
    [Headings.lastUpdated]: (<Fields.Text value={entry.last_updated} />),
